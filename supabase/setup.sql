...
DO $$ 
BEGIN
    -- Create product_storage table if it doesn't exist
    ...
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'stock_adjustments') THEN
...
-- Create the stock adjustment function
...
BEGIN
    ...
    IF _new_stock < 0 THEN
        RAISE EXCEPTION 'Stock tidak boleh negatif. Stok saat ini: %, Pengurangan: %', _current_stock, abs(_quantity);
    END IF;
