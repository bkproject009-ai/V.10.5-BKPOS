-- First drop the existing function if exists
...
-- Create new role check function with explicit parameter type
CREATE OR REPLACE FUNCTION check_user_role_v2(required_role text)
RETURNS boolean
...
ON products FOR ALL
TO authenticated
USING (check_user_role_v2('admin'));
...
USING (
    EXISTS (
        ...
        AND (cashier_id = auth.uid() OR check_user_role_v2('admin'))
    )
);
...
GRANT EXECUTE ON FUNCTION check_user_role_v2(text) TO authenticated;